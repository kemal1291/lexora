...
router.post('/register', [
  ...
  try {
    ...
    if (!errors.isEmpty()) return sendError(res, 'Data tidak valid', 422, errors.array());

    const { name, email, password, phone, nik, address } = req.body;

    const existingUser = await query('SELECT id FROM users WHERE email = $1', [email.toLowerCase()]);
    if (existingUser.rows.length > 0) return sendError(res, 'Email sudah terdaftar', 409);

    if (phone) {
      const existingPhone = await query('SELECT id FROM users WHERE phone = $1', [phone]);
      if (existingPhone.rows.length > 0) return sendError(res, 'Nomor telepon sudah terdaftar', 409);
    }

    const passwordHash = await bcrypt.hash(password, 12);
    const result = await query(
      `INSERT INTO users (name, email, phone, password_hash, nik, address, auth_provider)
       VALUES ($1, $2, $3, $4, $5, $6, 'email') RETURNING *`,
      [name, email.toLowerCase(), phone || null, passwordHash, nik || null, address || null]
    );

    const user = result.rows[0];
    const token = generateToken(user.id, 'user');
    return sendSuccess(res, formatUser(user, token), 'Registrasi berhasil', 201);
...
router.post('/login', [
  ...
  try {
    ...
    const user = result.rows[0];
    if (!user || !user.password_hash) return sendError(res, 'Email atau password salah', 401);

    const isValid = await bcrypt.compare(password, user.password_hash);
    if (!isValid) return sendError(res, 'Email atau password salah', 401);
...
router.post('/google', async (req, res, next) => {
  try {
    ...
    if (!firebaseToken) return sendError(res, 'Firebase token diperlukan', 400);
    ...
    if (userResult.rows.length > 0) {
      ...
      if (!user.firebase_uid) {
        await query('UPDATE users SET firebase_uid = $1, photo_url = $2 WHERE id = $3',
          [uid, picture, user.id]);
    ...
    if (userResult.rows.length > 0) {
      ...
      const newUser = await query(
        `INSERT INTO users (name, phone, firebase_uid, auth_provider, is_verified)
         VALUES ($1, $2, $3, 'phone', true) RETURNING *`,
...
router.get('/me', authenticate, async (req, res, next) => {
  try {
    ...
    if (!result.rows[0]) return sendError(res, 'User tidak ditemukan', 404);
...
// =============================================
// PUT /auth/profile
// Update profil + simpan FCM token
// =============================================
router.put('/profile', authenticate, [
  ...
  try {
    ...
    const { name, phone, nik, address, fcmToken } = req.body;
    const table = req.user.role === 'advocate' ? 'advocates' : 'users';
    ...
    const result = await query(
      `UPDATE ${table}
       ...
       WHERE id = $6
       RETURNING *`,
      [name, phone, nik, address, fcmToken ?? null, req.user.id]
    );

    if (fcmToken) console.log(`[FCM] Token user ${req.user.id} tersimpan`);
...
router.put('/change-password', authenticate, [
  ...
  try {
    ...
    if (!isValid) return sendError(res, 'Password lama tidak benar', 401);
...
router.post('/advocate/login', [
  ...
  try {
    ...
    if (!advocate || !advocate.password_hash) return sendError(res, 'Email atau password salah', 401);
    ...
    return sendSuccess(res, {
      token,
      advocate: {
        id: advocate.id, name: advocate.name, email: advocate.email,
        phone: advocate.phone, photoUrl: advocate.photo_url,
        title: advocate.title, firmName: advocate.firm_name,
        isVerified: advocate.is_verified, isAvailable: advocate.is_available,
        role: 'advocate',
      },
...
// =============================================
// POST /auth/users/fcm-token  ← dipindah SEBELUM module.exports
// =============================================
